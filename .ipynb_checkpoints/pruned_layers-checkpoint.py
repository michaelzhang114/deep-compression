...
class PruneLinear(nn.Module):
    ...
    def prune_by_percentage(self, q=5.0):
        ...
        --------------Your Code---------------------
        """
        w_np = self.linear.weight.data.cpu().numpy()
        threshold = np.percentile(np.absolute(w_np), q)
        indices = np.absolute(w_np) < threshold
        w_np[indices] = 0
        pruned_torch = torch.from_numpy(w_np).cuda()
        self.linear.weight.data = pruned_torch
        
        non_zero_params = np.sum(w_np != 0.0000)
        total_params = np.size(w_np)
        self.sparsity = 1.0 - non_zero_params / total_params
        self.mask = (w_np != 0.0000).astype(int)
...
class PrunedConv(nn.Module):
    ...
    def prune_by_percentage(self, q=5.0):
        ...
        --------------Your Code---------------------
        """
        w_np = self.conv.weight.data.cpu().numpy()
        threshold = np.percentile(np.absolute(w_np), q)
        indices = np.absolute(w_np) < threshold
        w_np[indices] = 0
        pruned_torch = torch.from_numpy(w_np).cuda()
        self.conv.weight.data = pruned_torch
