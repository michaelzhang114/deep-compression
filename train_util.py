...
def train(net, epochs=100, batch_size=128, lr=0.01, reg=5e-4, saving=True):
    """
    Training a network
    :param net:
    :param epochs:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_acc > best_acc and saving==True:
            best_acc = val_acc
            print("Saving...")
...
def train_iter(net, iter_num, epochs=100, batch_size=128, lr=0.01, reg=5e-4):
    """
    Training a network
    :param net:
    :param epochs:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            print("Saving...")
            dest = "net_before_pruning_iter_" + str(iter_num) + ".pt"
            torch.save(net.state_dict(), dest)            


def finetune_after_prune(net, epochs=100, batch_size=128, lr=0.01, reg=5e-4, saving=True):
    print('==> Preparing data..')
    transform_train = transforms.Compose([
        transforms.RandomCrop(32, padding=4),
        transforms.RandomHorizontalFlip(),
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            for m in net.modules():
                if isinstance(m, PrunedConv):
                    m.conv.weight.grad *= torch.from_numpy(m.mask).float().to(device)
                if isinstance(m, PruneLinear):
                    m.linear.weight.grad *= torch.from_numpy(m.mask).float().to(device)
